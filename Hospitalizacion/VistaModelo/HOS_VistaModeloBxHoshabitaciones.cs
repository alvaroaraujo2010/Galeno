...
namespace Hospitalizacion.VistaModelo
{
    ...
    /// </para>
    /// </summary>
    public class VistaModeloBxHoshabitaciones : VistaModeloBxHoshabitacionesBase
    {
        //-------------------------------------------------
            ...
            try
            {
                switch (tcrNombrePropiedad)
                {
                    ...
                    case "G1Hos_deshab_habi":
                        ...
                        {
                            G1Hos_deshab_habi = G1Hos_deshab_habi.ToUpper();
                            if (!Funciones.flgSoloTexto("AN", G1Hos_deshab_habi))
                            {
                                lcrValorReturn = "Dato no es valido";
                            }
                        }
                        break;
                    ...
                    case "G1Hos_tiphab_habi":
                        ...
                        {
                            if (!Funciones.flgExisteElemento(G1Hos_tiphab_habi,",", "1,2"))
                            {
                                lcrValorReturn = "Dato no es valido";
                            }
                        }
        ...
        public override string fcrValidacionRel(string tcrNombrePropiedad)
        {
            ...
            try
            {
                switch (tcrNombrePropiedad)
                {
                    ...
                    case "G2Hos_fechac_caho":
                        if (string.IsNullOrWhiteSpace(G2Hos_fechac_caho) ||
                            G2Hos_fechac_caho.Trim() == "//" || G2Hos_fechac_caho.Trim() == "/  /")
                        {
                            lcrValorReturn = "Fecha Prueba: Es requerido";
                        }
                        else
                        {
                            if (!Funciones.flgValidaFecha("DMY","/",G2Hos_fechac_caho))
                            {
                                lcrValorReturn = "Fecha Prueba: Valor Fecha no es valido";
                            }
                        }
                        break;

                    case "G2Hos_horapr_caho":
                        if (string.IsNullOrWhiteSpace(G2Hos_horapr_caho) ||
                            G2Hos_horapr_caho.Trim() == "::" || G2Hos_horapr_caho.Trim() == ":  :")
                        {
                            lcrValorReturn = "Hora  Prueba: Es requerido";
                        }
                        else
                        {
                            if (!Funciones.flgValidaHora(G2Hos_horapr_caho, "12", ":"))
                            {
                                lcrValorReturn = "Hora Prueba: Valor hora no es valido";
                            }
                        }
                        break;
