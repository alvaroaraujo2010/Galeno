...
namespace Sistema.Dian
{
    ...
    public class DocumentoNotaCredito
    {
        ...
        public TipoDocumentoElectronico Tipo { get; set; } = TipoDocumentoElectronico.NotaCredito;

        public ModeloSismaesterceros lobRegCliente { get; set; } // Cliente real 

        public TipoFirma TipoFirma { get; set; } = TipoFirma.NotaCrédito;

        public string TextoTipo { get; set; } = "91";
        ...
        public bool FlgCrearDocumento(ModeloFeFacturaMa toboRegFactura, out string mensaje)
        {
            ...
            var lobRegRefFact = ModeloFeFacturaMa.FobRegistroFcmfemaesfactefmaEx("ID", toboRegFactura.Fcm_sercre_mfac);
            ...
            AddressType obtenerAddress(string tcrDireccion, Municipio tobRegMunicipio)
            {
                ...
                { // 1..1 CAJ08.
                    ID = new IDType { Value = Validar(tobRegMunicipio.IDMunicipio, "5") }, // 1..1 CAJ09.
                    CityName = new CityNameType { Value = Validar(tobRegMunicipio.NombreMunicipio, "1..60", true) }, // 1..1 CAJ10.
                    // PostalZone = , // 0..1 CAJ73 T1..10. Código postal. Ver lista de valores posibles en el numeral 13.4.4. Por lo general no se tiene ni se usa.
                    CountrySubentity = new CountrySubentityType { Value = Validar(tobRegMunicipio.NombreDepartamento, "1..60", true) }, // 1..1 CAJ11.
                    CountrySubentityCode = new CountrySubentityCodeType { Value = Validar(tobRegMunicipio.CodigoDepartamento, "1..5") }, // 1..1 CAJ12.
                    AddressLine = new AddressLineType[1] { // 1..N FAJ13.
                    new AddressLineType { Line = new LineType { Value = Validar(tcrDireccion ?? " ", "1..300", true) } } // 1..1 CAJ14. Elemento de texto libre, que el emisor puede elegir utilizar para poner toda la información de su dirección, en lugar de utilizar elementos estructurados (los demás elementos de este grupo). Informar la dirección, sin ciudad ni departamento. Estos dos textos de la documentación de la DIAN son contradictorios entonces se escribirá la dirección en este campo y se completarán los otros normalmente. Debido a que es un elemento obligatorio de largo 1 si por alguna razón no se dispone de la dirección se enviará un espacio en blanco.
                },
                    Country = new CountryType
                    { // 1..1 CAJ15.
                        IdentificationCode = new IdentificationCodeType { Value = Validar(tobRegMunicipio.CodigoPais, "1..3") }, // 1..1 CAJ16. Debe informar literal "CO". El tamaño en la documentación dice 3 pero no puede ser 3, debe ser flexible para no contradecir el requerimiento de poner "CO".
                        Name = new NameType1
                        {
                            Value = Validar(tobRegMunicipio.NombrePais, "4..41", true), // 0..1 CAJ17. Debe informar el literal "Colombia".
                            languageID = Validar(tobRegMunicipio.CodigoLenguajePais, "2"), // 0..1 CAJ18.
                        },
                    },
                };
            ...
            //*************************************************
            var invoice = new CreditNoteType { UBLExtensions = new UBLExtensionType[2] }; // 1..1 FAA01. 1..1 FAA02. 
            ...
            var ublExtension1 = new UBLExtensionType(); // 1 de 2..N CAA01.    
            var extensionContentDian = new ExtensionContentDianType(); // 1..1 CAA01.
            ...
            var dianExtensions = new DianExtensionsType(); // 1..1 CAB03.
            ...
            invoice.UBLVersionID = new UBLVersionIDType { Value = Validar(VersionUBL, "7..8") }; // 1..1 CAD01.
            invoice.CustomizationID = new CustomizationIDType { Value = Validar(ObtenerCódigoTipoOperación(Tipo), "1..4") }; // 1..1 CAD02. 
            invoice.ProfileID = new ProfileIDType { Value = NombreYVersionNotaCredito }; // 1..1 CAD03. Según la documentación debería tener una longitud de 55 única pero el texto que dice en la documentación "Dian 2.1 Factura Electrónica de Venta" y el texto realmente exigido por el servicio web "Dian 2.1" contradicen esa restricción entonces no se validará su largo.
            invoice.ProfileExecutionID = new ProfileExecutionIDType { Value = Empresa.AmbienteFacturacionElectronica.AValor() }; // 1..1 CAD04.
            invoice.ID = new IDType { Value = Validar(lcrFacturaNumero, "1..20") }; // 1..1 CAD05.- Numero del documento (factura autorizada DIAN)

            invoice.UUID = new UUIDType
            {
                Value = Validar(CudeEfectivo, "96"), // 1..1 CAD06.
                schemeID = Empresa.AmbienteFacturacionElectronica.AValor(), // 1..1 CAD07.
                schemeName = Validar(AlgoritmoCude, "11"), // 1..1 CAD08.
            };

            invoice.IssueDate = new IssueDateType { Value = lcrFacturaFechaHora }; // 1..1 CAD09. Fecha emision del la Factura
            invoice.IssueTime = new IssueTimeType { Value = lcrFacturaFechaHora.ATexto(lcrFormatoHora) }; // 1..1 CAD10.
            ...
            invoice.CreditNoteTypeCode = new CreditNoteTypeCodeType { Value = TextoTipo }; // 1..1 CAD12.
            invoice.DocumentCurrencyCode = new DocumentCurrencyCodeType { Value = Validar(lcrMoneda, "3") }; // 1..1 CAD15 (En la documentación se saltan la FAD14)..
            invoice.LineCountNumeric = new LineCountNumericType { Value = Validar(lnuFacturaTotalRegistros, "1..6") }; // 1..1 CAD16.

            // CBF01 Se debe diligenciar únicamente cuando la FE se origina a partir de la correcció o ajuste que se da mediante un NC (nota credito)
            ...
            // CBG01 - Referencia a la factura que genera la nota credito
            ...
            { // 1..1 CBG02.
                ...
                { 
                    ID = new IDType { Value = Validar(lobRegRefFact.Fcm_numfac_mfac, "1..10") }, // 1..1 CBG03. En la documentación la restricción por tamaño es 10. Pero no tiene mucho sentido esta restricción porque siempre podrían ser más cortas entonces se usa 1..10.
                    UUID = new UUIDType
                    {
                        Value = Validar(lobRegRefFact.Fcm_idcufe_mfac, "96"), // 1..1 CBG04.
                        schemeName = Validar(AlgoritmoCufe, "11"), // 1..1 CBG05.
                    },
                    IssueDate = new IssueDateType { Value = lobRegRefFact.Fcm_fecfac_mfac }, // 0..1 CBG06.
                },
            ...
            { // 1..1 CAJ01.
                AdditionalAccountID = new AdditionalAccountIDType[1] { new AdditionalAccountIDType { Value = Empresa.TipoEntidad.AValor() } } // 1..1 CAJ02.
            };
            var supplierParty = new PartyType(); // 1..1 CAJ03.
            ...
            if (!string.IsNullOrEmpty(Empresa.NombreComercial))
            {
                supplierParty.PartyName = new PartyNameType[1] {
                    new PartyNameType { Name = new NameType1 { Value = Validar(Empresa.NombreComercial, "5..450", true) } } // 0..1 CAJ05. Name: 1..1 CAJ06.
                };
            }

            supplierParty.PhysicalLocation = new LocationType1
            {
                Address = obtenerAddress(Empresa.DirecciónUbicaciónEfectiva, Empresa.MunicipioUbicacion) // 0..1 CAJ07. Es opcional pero los campos más sujetos a errores que son los de nombre solo solo generan notificación en caso de no coincidir con los datos de la DIAN entonces se prefiere informar todo.
            };

            supplierParty.PartyTaxScheme = new PartyTaxSchemeType[1] { // 1..1 CAJ19.
                new PartyTaxSchemeType {
                    RegistrationName = registrationNameFacturador, // 1..1 CAJ20.
                    ...
                    TaxLevelCode = new TaxLevelCodeType {
                        ...
                        //Value = Validar("0-99", "1..30"), // 1..1 CAJ26. El tamaño la documentación dice 30 único pero ese valor no tiene sentido. Se usará 1..30.
                        //listName = "05", // Supuestamente CAJ27 es un elemento opcional, pero el servidor de la DIAN si lo exige. Se encontró que se debe usar 48 o 49 dependiendo de si es o no responsable de IVA aquí http://facturasyrespuestas.com/2225/inquietud-campo-taxlevelcode. Estos valores se pueden ver en el numeral 16.1.6. Modificación del anexo técnico (06-09-2019) de la documentación.
                    ...
                    TaxScheme = new TaxSchemeType { // 1..1 CAJ39.
                        ...
                        //ID = new IDType { Value = Validar(ObtenerCódigoTipoImpuesto(tipoImpuesto, forzar01: true), "2..10") }, // 1..1 CAJ40. Aunque la documentación dice tamaño 3..10 esto va en contradicción con el código "01" o "04" se usa 2..10. La documentación dice usar los valores de la tabla 13.2.6.2 pero a la vez restringe los valores a 01 o 04 sin especificar que se debe hacer en los casos de IVA y INC, por esto se usa el parámetro forzar01 = true en el que si es IVA e INC se devuelve el valor de IVA = 01.
                        //Name = new NameType1 { Value = Validar(ObtenerNombreImpuesto(tipoImpuesto, forzarIVA: true), "3..30") } // 1..1 CAJ41. Aunque la documentación dice tamaño 10..30 esto va en contradicción con el tamaño de "IVA", se usa 3..30.
            ...
            supplierParty.PartyLegalEntity = new PartyLegalEntityType[1] { // 1..1 FAJ42.
                new PartyLegalEntityType {
                    RegistrationName = registrationNameFacturador, // 1..1 CAJ43.
                    CompanyID = companyIDFacturador, // Se reusa el objeto creado anteriormente. Aquí se incluyen automáticamente los elementos CAJ44, CAJ45, CAJ46, CAJ47 y CAJ48.   
                    CorporateRegistrationScheme = new CorporateRegistrationSchemeType { // 0..1 CAJ49. Grupo de información de registro del emisor. 
                        ID = new IDType { Value = Validar(Empresa.PrefijoNotaCredito, "1..6") }, // ID: 0..1 CAJ50, prefijo de la facturación usada para el punto de venta. Aunque es opcional agrega para evitar que reporte notificación. Dice que el tamaño debe ser 6 pero claramente es incorrecto porque los prefijos pueden ser de menor tamaño, se valida con 1..6.
                        // Name = new NameType1 { Value = } // Name: 0..1 CAJ51 T9 Número de matrícula mercantil. Al ser opcional y al no haber claridad a que se refiere se omite.
                    }    
            ...
            { // 0..1 CAJ67. Aunque su uso no es obligatorio se considera apropiado informarlo porque es información que se dispone. Los clientes pueden hacer uso de esta información en la factura electrónica.
                Name = new NameType1 { Value = Empresa.NombreContactoFacturación }, // 0..1 CAJ68.
                Telephone = new TelephoneType { Value = Empresa.TeléfonoContactoFacturación }, // 0..1 CAJ69.
                // Telefax = new TelefaxType { Value = }, // 0..1 CAJ70. El fax ya es obsoleto, no se requiere informar.
                ElectronicMail = new ElectronicMailType { Value = Empresa.EmailContactoFacturación }, // 0..1 CAJ71.
                // Note = new NoteType { Value = "" }, // 0..1 CAJ72. No es necesario saturar la interfaz requiriendo notas sobre el contacto.
            };

            accountingSupplierParty.Party = supplierParty; // add
            ...
            { // 1..1 CAK01.
                AdditionalAccountID = new AdditionalAccountIDType[1] { new AdditionalAccountIDType { Value = lobRegCliente.Sis_tipper_sitr } } // 1..1 CAK02.
            };

            var customerParty = new PartyType()
            { // 1..1 FAK03.
                PartyIdentification = new PartyIdentificationType[1] {
                new PartyIdentificationType {  // 1..1 CAK03-1.
                    ID = new IDType {
                            Value =  Validar(lobRegCliente.Sis_FactEleNitAdquirente, "5..12"), // 1..1 CAK03-2.
                            schemeName = lobRegCliente.Sis_tipide_tido, // 1..1 CAK03-3.
                            schemeID = lobRegCliente.Sis_FactEleDigitoVerificNit, // 0..1 CAK03-4.
                        }
                    }
                }
            };

            if (!string.IsNullOrEmpty(lobRegCliente.Sis_FactEleRazonSocial)) // 0..1 CAK05.
            ...
            customerParty.PartyTaxScheme = new PartyTaxSchemeType[1] {
                new PartyTaxSchemeType { // 0..1 FAK19.
                    ...
                    TaxLevelCode = new TaxLevelCodeType {
                        ...
                        listName = "48",
                        /*
                        Value = Validar("0-99", "1..30"), // 1..1 CAK26. El tamaño la documentación dice 30 único pero ese valor no tiene sentido. Se usará 1..30.
                        listName = "05",
                        */
                    },
            ...
            customerParty.PartyTaxScheme[0].RegistrationAddress = obtenerAddress(lobRegCliente.Sis_direcc_sitr, lobRegCliente.Sis_MunicipioUbicacion); // 0..1 CAK28. Incluye CAK29, CAK30, CAK58, CFAK31, CAK32, CAK33, CAK34, CAK35, CAK36, CAK37 y CAK38.
            customerParty.PartyTaxScheme[0].TaxScheme = new TaxSchemeType
            { // 1..1 CAK39.
                //ID = new IDType { Value = Validar(ObtenerCódigoTipoImpuesto(tipoImpuesto), "2..10") }, // 1..1 CAK40. Dice que para el consumidor final debe informar "ZZ" pero quedan dudas al respecto porque no tiene mucho que ver con los impuestos de la factura, se dejará normal.
                //Name = new NameType1 { Value = Validar(tipoImpuesto.ATexto(), "3..30") } // 1..1 CAK41.
                ID = new IDType { Value = Validar("01", "2..10") }, // 1..1 CAK40. Dice que para el consumidor final debe informar "ZZ" pero quedan dudas al respecto porque no tiene mucho que ver con los impuestos de la factura, se dejará normal.
                Name = new NameType1 { Value = Validar("IVA", "3..30") } // 1..1 CAK41.

            }; // 1..1 CAK39. Incluye CAK40 y CAK41.

            customerParty.PartyLegalEntity = new PartyLegalEntityType[1] { // 1..1 CAK42.
                new PartyLegalEntityType {
                    RegistrationName = registrationNameCliente, // 1..1 CAK43.
                    CompanyID = companyIDCliente, // 1..1 CAK44. Incluye CAK45, CAK46, CAK47 y CAK48.
                    // CorporateRegistrationScheme =  // 0..1 CAK49. Incluye CAK50. Es opcional y no hay claridad a que se refiere entonces se omite.
                }
            };
            ...
            if (lobRegCliente.Sis_nomcon_sitr != null)
            {
                ...
                { // 0..1 CAK51.
                    Name = new NameType1 { Value = lobRegCliente.Sis_nomcon_sitr }, // 0..1 CAK52.
                    Telephone = new TelephoneType { Value = lobRegCliente.Sis_telefo_sitr }, // 0..1 CAK53.
                    // Telefax = new TelefaxType { Value = }, // 0..1 CAK54. Tecnología obsoleta, no es necesario.
                    ElectronicMail = new ElectronicMailType { Value = lobRegCliente.Sis_emailc_sitr }, // 0..1 FAK55.
                    // Note = new NoteType { Value = }, // 0..1 CAK56. No es necesario saturar la interfaz pidiendo este dato.

                    /*
                    Name = new NameType1 { Value = lcrDatosCliente.ContactoFacturas.Nombre }, // 0..1 CAK52.
                    Telephone = new TelephoneType { Value = lcrDatosCliente.ContactoFacturas.Teléfono }, // 0..1 CAK53.
                    // Telefax = new TelefaxType { Value = }, // 0..1 CAK54. Tecnología obsoleta, no es necesario.
                    ElectronicMail = new ElectronicMailType { Value = lcrDatosCliente.ContactoFacturas.Email }, // 0..1 CAK55.
                    // Note = new NoteType { Value = }, // 0..1 CAK56. No es necesario saturar la interfaz pidiendo este dato.
                    */
                };
            ...
            if (lobRegCliente.TipoEntidad != TipoEntidad.Empresa)
            {
                ...
                customerParty.Person = new PersonType[1] { // 0..1 CAK56.
                    new PersonType {
                        // ID = new IDType { Value = } // 0..1 CAK56-1.
                        FirstName = new FirstNameType { Value = NombreUsuarioFinal }, // 1..1 CAK56-2.
                        FamilyName = new FamilyNameType { Value = ApellidoUsuarioFinal }, // 0..1 CAK56-3.
                        MiddleName = new MiddleNameType { Value = lobRegCliente.Sis_FactEleRazonSocial }, // 0..1 CAK56-4. Aunque dice Middle Name en la documentación dice poner Nombre del adquiriente entonces se pondrá el nombre completo con apellidos.
                        ResidenceAddress = obtenerAddress(lobRegCliente.Sis_direcc_sitr, lobRegCliente.Sis_MunicipioUbicacion), // 0..1 CAK56-5, Incluye todos los otros CAK56 sin código.
                    }
                };
            ...
            // invoice.TaxRepresentativeParty = new PartyType { }; // 0..1 CAL01. Incluye CAL02, CAL03, CAL04, CA05, CAL07 y CAL06. No hay claridad a que se refieren con 'Persona autorizada para descargar documentos'. Al ser opcional se omite.
            ...
            */
            ...
            var paymentsMeans = new PaymentMeansType[1]; // 1..N CAN01.
            paymentsMeans[0] = new PaymentMeansType
            {
                ID = new IDType { Value = toboRegFactura.Fcm_forpag_mfac.Trim() }, // 1..1 CAN02.
                PaymentMeansCode = new PaymentMeansCodeType { Value = CodigoMedioPagoPorDefinir }, // 1..1 CAN03. En la documentación dice que el tamaño debe ser 1..2 pero CódigoMedioPagoPorDefinir es ZZZ, entonces no se realiza ninguna validación.
                // PaymentID = new PaymentIDType { Value = } // 0..N no hay claridad a que se refiere y es opcional. El CAN06 PaymentTerms ni está documentado.
            };

            if (Funciones.flgValidaFecha("DMY", "/", Funciones.fcrConvertFecha(toboRegFactura.Fcm_fecven_mfac)) == true)
            {
                paymentsMeans[0].PaymentDueDate = new PaymentDueDateType { Value = toboRegFactura.Fcm_fecven_mfac }; // 0..1 CAN04.
            }
            ...
            */
            ...
            var ldeBrutoYTributo = ldeValorBruto + ldeValorTributo;         // 1..1 CAU06 Valor Bruto mas Tirubutos solamente
            ...
            { // 1..1 CAU01.
                ...
                TaxInclusiveAmount = new TaxInclusiveAmountType
                {
                    ...
                    currencyID = lcrMoneda // 1..1 FAU06. Moneda: 1..1 CAU07. Total de valor bruto más tributos.
                },

                // No reportar en cero // 0..1 FAU08. currencyID 1..1 CAU09. Suma de todos los descuentos aplicados a nivel de la factura (son los de AllowanceCharge, es decir los condicionados).
                AllowanceTotalAmount = fobAllowanceTotalAmount(ldeDescuentos, lcrMoneda),

                // ChargeTotalAmount = new ChargeTotalAmountType { Value = 0, currencyID = Opciones.Moneda }; // 0..1 CAU10. currencyID 1..1 CAU11. Suma de todos los cargos aplicados a nivel de la factura. No se soportan cargos. La manera más fácil e intuitiva de generarlos facturando un servicio dentro de la factura.
                ...
                PayableAmount = new PayableAmountType
                {
                    ...
                    currencyID = lcrMoneda // 1..1 CAU14. El valor a pagar es igual a la suma del valor bruto + tributos - valor del descuento total + valor del cargo total - valor del anticipo total. Moneda: 1..1 CAU15. 
                }
            };
            ...
            static AllowanceTotalAmountType? fobAllowanceTotalAmount(decimal tdeValorDescuento, string tcrMoneda)
            {
                ...
                var lobAllowanceTotalAmount = new AllowanceTotalAmountType
                {
                    ...
                    currencyID = tcrMoneda // 0..1 CAU08. currencyID 1..1 CAU09. Suma de todos los descuentos aplicados a nivel de la factura (son los de AllowanceCharge, es decir los condicionados).
                };
            ...
            var invoiceLines = new CreditNoteLineType[toboRegFactura.tmpListVenta.Count];
            var lnuNumIDlinea = 1;

            foreach (var lobReg in toboRegFactura.tmpListVenta)
            { // 1..N CAV01.

                var invoiceLine = new CreditNoteLineType();
                ...
                invoiceLine.ID = new IDType { Value = Validar(lnuNumIDlinea.ATexto(), "1..4") }; // 1..1 CAV02.
                // invoiceLine.Note = new NoteType[] { new NoteType { Value = , "20..5000", true) } }; // 0..N CAV03. Una nota cualquiera sobre la línea. No se agrega para evitar saturar la interfaz pidiendo esta información.
                invoiceLine.CreditedQuantity = new CreditedQuantityType
                {
                    Value = lobReg.Fcm_totuni_dfac, // 1..1 CAV04.
                    unitCode = "94" //Validar(lobReg.Sis_codume_sium, "2..5"), // 1..1 CAV05.  
                };
                invoiceLine.LineExtensionAmount = new LineExtensionAmountType
                {
                    Value = Validar((decimal)lobReg.Fcm_valbru_dfac, "0..15 p (2..6)", 2), // 1..1 CAV06.
                    currencyID = lcrMoneda // currencyID: 1..1 CAV07. 
                };

                invoiceLine.PricingReference = new PricingReferenceType
                { // 0..1 CAW01. Obligatorio informar si se trata de muestras comerciales.
                    AlternativeConditionPrice = new PriceType[] {  // 1..1 CAW02.
                        new PriceType {
                            PriceAmount = new PriceAmountType {
                                Value = Validar((decimal)lobReg.Fcm_valser_mant, "0..15 p (2..6)", 2), // 1..1 CAW03.
                                currencyID = lcrMoneda // currencyID: 1..1 CAW04.
                            },
                            PriceTypeCode =  new PriceTypeCodeType { Value = CódigoPrecioReferencia }, // 1..1 CAW05.
                        }
                    },
                };

                invoiceLine.AllowanceCharge = new AllowanceChargeType[] {
                    new AllowanceChargeType { // 0..N CBE01. Este grupo se debe informar a nivel de ítem si el cargo o descuento afecta la base gravable del ítem (Es decir es un descuento comercial).
                        ID = new IDType { Value = "1" }, // CBE02.
                        ChargeIndicator = new ChargeIndicatorType { Value = false }, // 1..1 CAB03.
                        // AllowanceChargeReason = , // 0..1 CBE04. Por lo general es un descuento comercial. No se saturará la interfaz requiriendo esta información.
                        MultiplierFactorNumeric = new MultiplierFactorNumericType {
                            Value = Validar(0, "1..6 p (0..2)", 2) // 1..1 CBE05. Aunque debería ser línea.PorcentajeDescuentoComercial * 100, por simplicidad al informar a la DIAN no se reportará porque este está incluído en el SubtotalBase.
                        },
                        Amount = new AmountType2 {
                            Value = Validar(0, "4..15 p (2..6)", 2), currencyID = lcrMoneda // 1..1 CBE06. currencyID: 1..1 CBE07. Aunque debería ser línea.DescuentoComercial, por simplicidad al informar a la DIAN no se reportará porque este está incluído en el SubtotalBase.
                        },
                        BaseAmount = new BaseAmountType {
                            Value = Validar((decimal)lobReg.Fcm_valbru_dfac, "4..15 p (2..6)", 2), currencyID = lcrMoneda // 1..1 CBE08. currencyID: 1..1 CBE09.
                        },
                    }
                };
                ...
                { // FAX01 no debe ser informado para facturas del régimen simple grupo I ni para ítems cuyo concepto en contratos de AIU no haga parte de la base gravable.
                    invoiceLine.TaxTotal = FobGenerarTaxTotales(lobReg);  // 0..N CAX01.
                }
                // invoiceLine.WithholdingTaxTotal = ; // 0..N CAY01. Igual que en CAT01. Incluye CAY02, CAY03, CAY04, CAY05, CAY06, CAY07, CAY08, CAY09, CAY10, CAY11, CAY12 y CAY13.

                invoiceLine.Item = new ItemType
                {
                    Description = new DescriptionType[] { new DescriptionType { Value = Validar(lobReg.Fcm_desser_dfac, "5..300", true) } }, // 1..3 CAZ02.
                    ...
                    PackSizeNumeric = 1 >= 1000 ? null : new PackSizeNumericType { Value = 1 }, // 0..1 CAZ03. Como la documentación de la DIAN restringe este campo a 3 números cuando la unidad sea mayor o igual a 1000 se omite.
                    ...
                    { // 1..N CAZ09.
                        ...
                        { // 1..1 CAZ10.
                            ...
                            schemeID = CódigoEstándarAdopciónContribuyente, // 1..1 CAZ12. Para 0..1 CAZ13 no dice nada la documentación.
                    ...
                    // AdditionalItemProperty = new ItemPropertyType { }; // 0..N FBF01. Incluye CBF02 y CBF03. Grupo de información para adicionar información específica del ítem que puede ser solicitada por autoridades o entidades diferentes a la DIAN. Se omite porque no se sabe en que casos es necesario.
                    // InformationContentProviderParty = new PartyType { }; // 0..1 FBA01. Incluye CBA02, CBA03, CAB04, CAB05, CAB06, FAB09, FAB07 y FBA08. Grupo de información que describen el mandante de la operación de venta. Aplica solo para mandatos y se debe informar a nivel de ítem. SimpleOps aún no soporta este tipo de operaciones, se omite.
            ...
            espaciosNombres.Add("schemaLocation", "urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2     " + "http://docs.oasis-open.org/ubl/os-UBL-2.1/xsd/maindoc/UBL-CreditNote-2.1.xsd");

            var serializadorXml = new XmlSerializer(typeof(CreditNoteType), new Type[] {
            ...
            decimal tdePorcentaje)
        {
            ...
            taxSubtotal.TaxableAmount = new TaxableAmountType
            {
                ...
                currencyID = lcrMoneda // 1..1 CAS05, CAX05. currencyID: 1..1 CAS06, CAX06. En el caso de que el tributo sea una porcentaje del valor tributable informar la base imponible en valor monetario.
            };
            taxSubtotal.TaxCategory.Percent = new PercentType1
            {
                Value = Validar(tdePorcentaje, "0..5 p (0..3)", 2) // 0..1 CAS14, CAX14. Aunque en la tabla 13.3.9 no mencionan el 0 como un valor válido para el INC se permitirá el 0 porque se muestra en el ejemplo de XML.
            };
