...
namespace Sistema.Dian
{
    ...
    public class DocumentoFactura
    {
        ...
        /// <summary>
        /// Temporal resumen tipos de Impuestos
        /// </summary>
        ...
        /// </summary>
        public bool FlgCrearDocumento(ModeloFeFacturaMa tobRegFact, out string mensaje)
        {
            ...
            // Datos de la factura
            var lcrFacturaNumero           = tobRegFact.Fcm_numfac_mfac;
            var lnuFacturaTotalRegistros   = tobRegFact.tmpListVenta.Count;
            var lcrFacturaFechaHora        = Funciones.FdaConcatenarFechaYHora(tobRegFact.Fcm_fecfac_mfac, tobRegFact.Fcm_horfac_mfac);

            var lcrFacturaFechaVencimiento = tobRegFact.Fcm_fecven_mfac;
            lobRegCliente                  = tobRegFact.lobRegCliente;
            TextoTipo                      = FcrSeleccionInvoiceTypeCode(Tipo);

            CudeEfectivo = FcrGenerarCude(tobRegFact, llgEsCufe);
            DocGenNombreCodigoCufe = CudeEfectivo;

            Empresa.FcvCargarResolucionFacturacion(tobRegFact.lobRegResolDian);
            ...
            AddressType obtenerAddress(string tcrDireccion, Municipio tobRegMunicipio)
            {
                ...
                { // 1..1 FAJ08.
                    ID = new IDType { Value = Validar(tobRegMunicipio.IDMunicipio, "5") }, // 1..1 FAJ09.
            ...
            var companyIDCliente = new CompanyIDType
            {
                ...
                schemeID = tobRegFact.lobRegCliente.Sis_FactEleDigitoVerificNit, // 0..1 FAK24.
                schemeName = tobRegFact.lobRegCliente.TipoNit // 1..1 FAK25.
            };
            ...
            //var ldeNumeroAutorizacion = Convert.ToDecimal(tobRegFact.lobRegResolDian.Fcm_numres_srfa);
            ...
            { // 1..1 FAB13.
                IdentificationCode = new IdentificationCodeType
                {
                    ...
                    listSchemeURI = UriEsquemaIdentificacionPais // 1..1 FAB17.
                }
            };
            ...
            { // 1..1 FAB30.
                AuthorizationProviderID = new coID2Type
                {
                    ...
                    schemeID = DigitoVerificacionNitDian, // 1..1 FAB34
            ...
            dianExtensions.QRCode = FcrGenerarCodigoQR(tobRegFact, CudeEfectivo, out _); // 1..1 FAB36.
            ...
            invoice.UUID = new UUIDType
            {
                ...
                schemeID = Empresa.AmbienteFacturacionElectronica.AValor(), // 1..1 FAD07.
            ...
            */
            ...
            if (Tipo == TipoDocumentoElectronico.FacturaContingenciaFacturador)
            ...
            */
            ...
            //MessageBox.Show("FlgCrearDocumento aqui voy 4");
            ...
            */
            ...
            //MessageBox.Show("FlgCrearDocumento aqui voy 7");
            ...
            {
                ID = new IDType { Value = tobRegFact.Fcm_forpag_mfac.Trim() }, // 1..1 FAN02.
            ...
            if (Funciones.flgValidaFecha("DMY", "/", Funciones.fcrConvertFecha(tobRegFact.Fcm_fecven_mfac)) == true)
            {
                paymentsMeans[0].PaymentDueDate = new PaymentDueDateType { Value = tobRegFact.Fcm_fecven_mfac }; // 0..1 FAN04.
            }
                
            invoice.PaymentMeans = paymentsMeans;


            // ANTICIPOS RECIBIDOS (PARA GALENO SON COPAGOS CUOTAS MODERADORAS Y CARGOS A USUARIOS)
            var ldePagosAnticipo = (decimal)(tobRegFact.Fcm_valcpa_dfac + tobRegFact.Fcm_valcmo_dfac + tobRegFact.Fcm_valusu_dfac);

            if (ldePagosAnticipo > 0)
            {
                var lcrDescAnticipo = (tobRegFact.Fcm_valcpa_dfac > 0 || tobRegFact.Fcm_valcmo_dfac > 0) ?
                                                                                "COPAGO RECIBIDO": "ANTICIPO DEL USUARIO";
                var prepaidPayments = new PaymentType[1]; // 0..N FBD01.
                prepaidPayments[0] = new PaymentType
                {
                    ID = new IDType { Value = Validar(tobRegFact.Fcm_secreg_mfac, "1..150", true) }, // 1..1 FBD02.
                    PaidAmount = new PaidAmountType { Value = Validar(ldePagosAnticipo, "4..15 p (2..6)", 2), currencyID = lcrMoneda }, // 1..1 FBD03. Incluye FBD04.
                    ReceivedDate = new ReceivedDateType { Value = tobRegFact.Fcm_fecfac_mfac }, // 1..1 FBD05.
                    PaidDate = new PaidDateType { Value = tobRegFact.Fcm_fecfac_mfac }, // 0..1 FBD06 No se registra en la base de datos la fecha de realización del pago, se registra es la fecha de recepción. Lo importante para efectos de la empresa es cuando se recibió. Es opcional, entonces no se registra.
                    // PaidTime = , // 0..1 FBD07. Igual que el anterior.
                    InstructionID = new InstructionIDType { Value = lcrDescAnticipo }, // 0..1 FBD08. Instrucciones relativas al pago T15..5000. Las observaciones del pago se consideran información de uso privado de la empresa y no se comparten con los clientes en la factura electrónica.
                };
            ...
            */
            ...
            invoice.TaxTotal = FobGenerarResumenImpuestos(tobRegFact);
            //invoice.TaxTotal = FobGenerarTaxTotalesMA(tobRegFact, 21 ); // ojo- 21% es de prueba
            ...
            //MessageBox.Show("FlgCrearDocumento aqui voy 9");
            //*************************************************
            // DATOS DE TOTALES
            //*************************************************
            #region Datos Totales

            var ldeValorBruto    = (decimal)tobRegFact.Fcm_valbru_dfac;
            var ldeValorTributo  = (decimal)tobRegFact.Fcm_valiva_dfac; // solo IVA por el moento
            var ldaValorBaseImp  = (decimal)tobRegFact.Fcm_valbsi_dfac;
            var ldeAnticipo      = (decimal)(tobRegFact.Fcm_valcpa_dfac + tobRegFact.Fcm_valcmo_dfac + tobRegFact.Fcm_valusu_dfac);
            var ldeDescuentos    = (decimal)tobRegFact.Fcm_valdes_dfac;
            var ldeBrutoYTributo = ldeValorBruto + ldeValorTributo;         // 1..1 FAU06 Valor Bruto mas Tirubutos solamente
            var ldeValorAPagar   = ldeValorBruto;
            //var ldeValorAPagar = (decimal)tobRegFact.Fcm_valfac_dfac;
            //var ldeValorAPagar = (decimal)tobRegFact.Fcm_valfac_dfac - ldeAnticipo;
            ...
            static PrepaidAmountType? fobPrepaidAmount(decimal tdeValorAnticipo, string tcrMoneda)
            {
                ...
                //MessageBox.Show("copago "+ tdeValorAnticipo);
            ...
            var invoiceLines = new InvoiceLineType[tobRegFact.tmpListVenta.Count];
            var lnuNumIDlinea = 1;
            ...
            { // 1..N FAV01.
                ...
                invoiceLine.InvoicedQuantity = new InvoicedQuantityType
                {
                    ...
                    unitCode = "94"// Validar(lobReg.Sis_codume_sium, "2..5"), // 1..1 FAV05.  
                };
                invoiceLine.LineExtensionAmount = new LineExtensionAmountType
                {
                    ...
                    Value = Validar((decimal)lobReg.Fcm_valbru_dfac, "0..15 p (2..6)", 2), // 1..1 FAV06.
                ...
                {
                    Description = new DescriptionType[] { new DescriptionType { Value = Validar(lobReg.Fcm_desser_dfac, "5..300", true) } }, // 1..3 FAZ02.
            ...
            invoice.InvoiceLine = invoiceLines; // ojo  activar solo para generar error de envio
        ...
        #region FobGenerarResumenImpuestosxx: resumen general impuestos
        /// <summary>
        /// Generar resumen general segun tipos y porcentajes de impuestos
        /// </summary>
        public TaxTotalType[]? FobGenerarResumenImpuestosxx(ModeloFeFacturaMa tobRegFact)
        {
            ...
            foreach (var lobReg in tobRegFact.tmpListVenta)
            {
                ...
                if (lobReg.Fcm_valiva_dfac > 0)
                {
                    ...
                    lobIVA.Tipo = "01"; // para IVA
                    lobIVA.Nombre = "IVA "+ lobIVA.Porcentaje.ToString();
                    lobIVA.Porcentaje = (decimal)lobReg.Fcm_poriva_dfac;
                    lobIVA.ValorTotal += (decimal)lobReg.Fcm_valiva_dfac;
                    lobIVA.ValorBase += (decimal)lobReg.Fcm_valbsi_dfac;

                    // Agregar a la lista
                    if (llgAdd == true) { tmpResumImp.Add(lobIVA); }
                }
                #endregion IVA>
            }
            ...
            {
                taxTotal = FobOtenerTaxTotal(lobReg.Tipo, "IVA",
                                             lobReg.ValorTotal,
                                             lobReg.ValorBase,
                                             lobReg.Porcentaje);
        ...
        #region FobGenerarTaxTotalesMAxx Maestro
        /// <summary>
        ...
        /// <returns></returns>
        public static TaxTotalType[]? FobGenerarTaxTotalesMAxx(ModeloFeFacturaMa tobRegistro, decimal tcrPorcentaje)
        {
        ...
        /// <returns></returns>
        public static TaxTotalType? FobOtenerTaxTotalxx(
        ...
        /// <returns></returns>
        public static TaxTotalType FobTaxTotalTypexx(decimal tdeTotalImpuesto, string tcrMoneda)
        {
        ...
        /// <param name="tcrMoneda">Tipo moneda para colombia es "COP"</param>
        /// <returns></returns>
        public static TaxSubtotalType FobTaxSubtotalTypexx(string tcxrTipo, string tcrNombre, decimal tdeTotalImpuesto, string tcrMoneda)
        {
