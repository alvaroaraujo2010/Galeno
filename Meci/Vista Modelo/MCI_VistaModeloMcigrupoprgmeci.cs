...
namespace Meci.VistaModelo
{
    ...
    /// </para>
    /// </summary>
    public class VistaModeloMcigrupoprgmeci : VistaModeloMcigrupoprgmeciBase
    {
        //-------------------------------------------------
            ...
            try
            {
                switch (tcrNombrePropiedad)
                {
                    ...
                    case "G1Mci_idesec_mcpa":
                        ...
                        {
                            var tmp = MCIValidarCodigo.fobRegBuscarMciparametrmeci(G1Mci_idesec_mcpa);
                            if (tmp != null && !String.IsNullOrWhiteSpace(tmp.mci_idesec_mcpa))
                            {
                                G1Mci_idesec_mcpl = tmp.mci_idesec_mcpl;
                                G1Mci_idesec_mcmo = tmp.mci_idesec_mcmo;
                                G1Mci_idesec_mcco = tmp.mci_idesec_mcco;
                                G1Mci_despar_mcpa = tmp.mci_despar_mcpa;
                            }
                            else
                            {
                    ...
                    case "G1Mci_etqgrp_mcgr":
                        #region MCI_ETQGRP_MCGR: Etiqueta Grupo
                        lcrNombreCampo = "Etiqueta Grupo";
                        lcrValorReturn = String.Empty;
                        lcrCodigoError = "A06";
                        if (String.IsNullOrWhiteSpace(G1Mci_etqgrp_mcgr))
                        {
                            lcrValorReturn = lcrNombreCampo + ": Es requerido";
                        }
                        else
                    ...
                    case "G1Mci_desgrp_mcgr":
                        #region MCI_DESGRP_MCGR: Descripción Grupo
                        lcrNombreCampo = "Descripción Grupo";
                        lcrValorReturn = String.Empty;
                        lcrCodigoError = "A07";
                        if (String.IsNullOrWhiteSpace(G1Mci_desgrp_mcgr))
                        {
                            lcrValorReturn = lcrNombreCampo + ": Es requerido";
                        }
                        else
                    ...
                    case "G1Mci_estreg_mcgr":
                        #region MCI_ESTREG_MCGR: Estado del grupo
                        lcrNombreCampo = "Estado del grupo";
                        lcrValorReturn = String.Empty;
                        lcrCodigoError = "A10";
                        if (String.IsNullOrWhiteSpace(G1Mci_estreg_mcgr))
                        {
                            lcrValorReturn = lcrNombreCampo + ": Es requerido";
                        }
                        else
                        {
                            if (!Funciones.flgExisteElemento(G1Mci_estreg_mcgr, ",", "1,2"))
                            {
